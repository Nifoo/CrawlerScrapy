...
class JobboleSpider(scrapy.Spider):
    ...
    def parse(self, response):
        ...
        detailedUrls = response.css("#archive .post.floated-thumb .post-thumb a::attr(href)").extract()
        for detailedUrl in detailedUrls:
            #print(detailedUrl)
            ...
            yield Request(parse.urljoin(response.url, detailedUrl), callback=self.parse_detail)
    ...
    def parse_detail(self, response):
        ...
        thumbUp = response.xpath("//span[contains(@class, 'vote-post-up')]/h10/text()").extract()[0]
